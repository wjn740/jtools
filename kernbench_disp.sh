...
do
	...
	#所以我们需要把数据转化成一行一行的
	for i in $(cat ${y} | egrep "^Elapsed Time | ^System Time" | awk '{print $3}');do echo $i >> ${new_dir}/data_group/${y}/${y}.${x}; echo $i >> ${new_dir}/data_group/${y}/line${j};((j++));done
	done
	popd
done
